...
// Buscar dados do trem para edição
$trem = null;
if (isset($_GET['id'])) {
    try {
        $query = "SELECT * FROM trens WHERE id_trem = :id";
        $stmt = $db->prepare($query);
        $stmt->bindParam(":id", $_GET['id']);
        $stmt->execute();
        $trem = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$trem) {
            header("Location: lista_trens.php");
            exit();
        }
    } catch (PDOException $e) {
        $error = "Erro ao carregar trem: " . $e->getMessage();
    }
} else {
    header("Location: lista_trens.php");
    exit();
...
if ($_POST) {
    try {
        ...
        if (empty($errors)) {
            ...
            // Bind parameters
            $stmt->bindParam(":id_trem", $id_trem);
            ...
            if ($stmt->execute()) {
                ...
                $trem = array_merge($trem, $_POST);
            } else {
                $message = "Erro ao atualizar trem. Tente novamente.";
                $message_type = "danger";
            }
        } else {
...
<head>
    ...
    <title>Guia Trem - Editar Trem</title>
...
<body>
                ...
                <div class="col-12 col-lg-10 col-xl-8">
                    <div class="form-container">
                        <h1 class="form-title text-center">
                            <i class="fas fa-edit me-2"></i>Editar Trem
                        </h1>
                        ...
                        <form method="POST" id="tremForm">
                            <input type="hidden" name="id_trem" value="<?php echo $trem['id_trem']; ?>">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" 
                                           ...
                                           id="numero_trem" 
                                           name="numero_trem" 
                                           value="<?php echo htmlspecialchars($trem['numero_trem']); ?>"
                                           maxlength="20"
                                           required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" 
                                           ...
                                           id="linha" 
                                           name="linha" 
                                           value="<?php echo htmlspecialchars($trem['linha']); ?>"
                                           maxlength="50"
                                           required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" 
                                           ...
                                           id="capacidade" 
                                           name="capacidade" 
                                           value="<?php echo $trem['capacidade']; ?>"
                                           min="1"
                                           max="1000"
                                           required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select" id="status_trem" name="status_trem" required>
                                        <option value="ativo" <?php echo $trem['status_trem'] == 'ativo' ? 'selected' : ''; ?>>Ativo</option>
                                        <option value="manutencao" <?php echo $trem['status_trem'] == 'manutencao' ? 'selected' : ''; ?>>Manutenção</option>
                                        <option value="inativo" <?php echo $trem['status_trem'] == 'inativo' ? 'selected' : ''; ?>>Inativo</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" 
                                           ...
                                           value="<?php echo $trem['data_ultima_manutencao'] ? $trem['data_ultima_manutencao'] : ''; ?>">
                                ...
                                <div class="col-12">
                                    <div class="card bg-light">
                                        <div class="card-body">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <small><strong>ID do Trem:</strong> <?php echo $trem['id_trem']; ?></small>
                                                </div>
                                                <div class="col-md-6">
                                                    <small><strong>Data de Criação:</strong> <?php echo date('d/m/Y H:i', strtotime($trem['data_criacao'])); ?></small>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="row mt-4">
                                <div class="col-12 d-flex justify-content-between">
                                    <a href="lista_trens.php" class="btn btn-cancel">
                                        <i class="fas fa-arrow-left me-2"></i>Voltar para Lista
                                    </a>
                                    <div>
                                        <button type="button" class="btn btn-cancel me-2" onclick="limparAlteracoes()">
                                            <i class="fas fa-times me-2"></i>Cancelar Alterações
                                        </button>
                                        <button type="submit" class="btn btn-custom">
                                            <i class="fas fa-save me-2"></i>Atualizar Trem
                                        </button>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        const valoresOriginais = {
            numero_trem: document.getElementById('numero_trem').value,
            modelo: document.getElementById('modelo').value,
            linha: document.getElementById('linha').value,
            capacidade: document.getElementById('capacidade').value,
            status_trem: document.getElementById('status_trem').value,
            data_ultima_manutencao: document.getElementById('data_ultima_manutencao').value
        };

        // Limpar alterações (restaurar valores originais)
        function limparAlteracoes() {
            if (confirm('Tem certeza que deseja descartar todas as alterações?')) {
                document.getElementById('numero_trem').value = valoresOriginais.numero_trem;
                document.getElementById('modelo').value = valoresOriginais.modelo;
                document.getElementById('linha').value = valoresOriginais.linha;
                document.getElementById('capacidade').value = valoresOriginais.capacidade;
                document.getElementById('status_trem').value = valoresOriginais.status_trem;
                document.getElementById('data_ultima_manutencao').value = valoresOriginais.data_ultima_manutencao;
